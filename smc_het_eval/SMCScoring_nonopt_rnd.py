...
def validate2A(data, nssms, return_ccm=True, mask=None, filter_mut=None):
    ...
    data = filter(None, data)
    if filter_mut != None:
        data = list(np.array(data)[filter_mut])
...
#        print "DEBUG worst_scores:", worst_scores
...
#            return verified, raw
            pred = validate2A(data1, truth_size, filter_mut=filter_mut)
            truth = validate2A(data2, truth_size, filter_mut=None)
            return truth, pred
...
#                    vout, raw = verify2A(predfile, truthfile, "Combined truth and pred file for Challenge 2A", *vcfargs, filter_mut=nssms[2], mask=masks['truths'], subchallenge="3A")
                    raw, pred = verify2A(predfile, truthfile, "Combined truth and pred file for Challenge 2A", *vcfargs, filter_mut=nssms[2], mask=masks['truths'], subchallenge="3A")
                except SampleError as e:
                    raise e
