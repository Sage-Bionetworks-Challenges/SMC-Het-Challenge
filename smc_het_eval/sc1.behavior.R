...
for(metric in metrics.1A){
  ...
  da = read.csv(paste(tsv_dir, "scoring1A_behavior_", metric, ".tsv", sep=""),sep="\t",header=FALSE)
  colnames(da) = c("Real","Pred","Error")
  
  # create the plot (all other sections follow the same format)
  png(file=paste(plot_dir, "1A_", metric, ".png", sep="")) # plot filepath
  print(
...
for(metric in metrics.1B){
  db = read.csv(paste(tsv_dir, "scoring1B_behavior_", metric, ".tsv", sep=""),sep="\t",header=FALSE)
  colnames(db) = c("Real","Pred","Error")
  
  png(file=paste(plot_dir, "1B_", metric, ".png", sep="")) # plot filepath
  print(
...
for(metric in metrics.1C){
  ...
  d = read.csv(paste(tsv_dir, "scoring1C_phi_sys_behavior_", metric, ".tsv", sep=""),sep="\t",header=FALSE)
  colnames(d) = c("Error","Metric")
  
  png(file=paste(plot_dir, "1C_phi_systematic_", metric, ".png", sep=""))
  print(
  ...
  d = read.csv(paste(tsv_dir, "scoring1C_phi_ZM_behavior_", metric, ".tsv", sep=""),sep="\t",head=FALSE)
  colnames(d) = c("Concentration","PhisUsed","Metric")
  
  png(file=paste(plot_dir, "1C_phi_ZM_", metric, ".png", sep=""))
  print(
  ...
  # concentration parameter (which determines the size of the error)
  d = read.csv(file=paste(tsv_dir, "scoring1C_nssm_behavior_", metric, ".tsv", sep=""), sep="\t",header=FALSE)
  colnames(d) = c("Concentration","NssmsUsed","Metric")
  
  png(file=paste(plot_dir, "1C_nssms_ZM_", metric, ".png", sep=""))
  print(
    ggplot(d, aes(x=as.ordered(Concentration), y=as.numeric(Metric))) + 
      geom_jitter(aes(color=as.ordered(Concentration)),position = position_jitter(height = 0, width=0.05)) +
      stat_summary(fun.y=median, fun.ymin=median, fun.ymax=median, geom="crossbar", width=0.7) +
  ...
  d = read.csv(file=paste(tsv_dir, "scoring1C_cases_", metric, ".tsv", sep=""), sep="\t",header=FALSE)
  colnames(d) = c("Case","Metric")
  
  png(file=paste(plot_dir, "1C_Cases_", metric, ".png", sep=""))
  print(
